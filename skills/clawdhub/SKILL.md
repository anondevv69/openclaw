...
```
When the user gives a ClawdHub URL (e.g. https://clawhub.ai/Andretuta/polymarket-agent), try the slug from the search result: often just the skill name (e.g. `polymarket-agent`). Run from **workspace root**: `npx clawdhub install polymarket-agent`. If that fails with "Skill not found", try `User/skill-name` (e.g. `Andretuta/polymarket-agent`). Run `npx clawdhub search "query"` first to see exact names/slugs.
